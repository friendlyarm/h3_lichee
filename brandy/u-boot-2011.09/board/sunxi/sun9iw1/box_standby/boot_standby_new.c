...
#define SCRIPT_ADDR				(0x44000000)
#define BOOT_STANDBY_RUN_ADDR	(0x08100000)
#define SRAM_A2_SIZE			(160 * 1024)
...
static int read_start_mode(void)
{
	...
	//*(volatile unsigned int *)(0x08001400 + 0x1f0) = (0x3 << 16);
	//mode = (*(volatile unsigned int *)(0x08001400 + 0x1f0)) & 0xff;
...
static void write_start_mode(int mode)
{
	...
	//printf("write_start_mode start\n");
	do {
		smc_writel((0x3 << 16) | (value << 8), SUNXI_RPRCM_BASE + 0x1f0);
		smc_writel((0x3 << 16) | (value << 8) | (1U<<31), SUNXI_RPRCM_BASE + 0x1f0);
		__usdelay(10);
		CP15ISB;
		CP15DMB;
		smc_writel((0x3 << 16) | (value<<8), SUNXI_RPRCM_BASE + 0x1f0);
	...
	} while ((reg_val & 0xff) != value);
	//printf("write_start_mode end\n");
}
...
static void init_cpus(void)
{
	...
	value = smc_readl(SUNXI_RPRCM_BASE + 0x0);
	value &= ~1;
	smc_writel(value, SUNXI_RPRCM_BASE + 0x0);
	value = smc_readl(SUNXI_RPRCM_BASE + 0x0);
	value |= 1;
	smc_writel(value, SUNXI_RPRCM_BASE + 0x0);
}
