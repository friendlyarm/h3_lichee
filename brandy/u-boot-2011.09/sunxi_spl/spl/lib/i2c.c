...
void i2c_init(uint bus_id, int speed, int slaveaddr)
{
	...
	int reg_value = 0;
	reg_value = readl(0x01c20800 + 0x24);
	reg_value &= 0x00ffffff;
	reg_value |= 0x22000000;
	writel(reg_value ,(0x01c20800 + 0x24));
	__usdelay(500);
	printf("i2c gpio config %x\n",readl(0x01c20800 + 0x24));
#else
	printf("need to set i2c gpio mulsel\n");
#endif
